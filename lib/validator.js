...
class Validator {
  ...
  _validate(label, key, rule, obj) {
    // 默认都是非必填，值空跳过不验证，如果有 required 验证器，则必填，空值将验证不通过
    if (rule.type != 'required' && is.empty(obj[key])) { return; }
...
function lengthValidator(label, rule, val) {
  ...
  switch (rule.rules[0]) {
    case '~':
      ...
      if (rule.rules[1] > rule.rules[2]) { throw new TypeError('rules[1] should be <= rules[2]'); }
